...
void fill(distributed_parallel_tag&& policy, ForwardIt first, ForwardIt last,
  ...
  distributed_map_void(
      ...
      first, last,
      // kernel
      [](rt::Handle& h, ForwardIt first, ForwardIt last, const T& value) {
...
void block_contiguous_local_par(rt::Handle& h, ForwardIt1 first,
  ...
  local_map_void_offset(h,
                        // range
                        first, last,
                        // kernel
                        [&](ForwardIt1 b, ForwardIt1 e, offset_t offset) {
                          std::transform(b, e, local_d_range.begin() + offset,
                                         op);
                        });
}
...
template <class ForwardIt1, class ForwardIt2, class UnaryOperation>
void block_contiguous_remote(rt::Handle& h, rt::Locality l, ForwardIt1 first,
                             ForwardIt1 last, ForwardIt2 d_first,
                             UnaryOperation op) {
  using itr_traits2 = distributed_iterator_traits<ForwardIt2>;
...
// synchronous
template <class ForwardIt1, class ForwardIt2, class UnaryOperation>
void block_contiguous_remote_par(rt::Handle& h, rt::Locality l,
...
ForwardIt2 dseq_kernel(std::true_type, ForwardIt1 first, ForwardIt1 last,
  ...
  for (auto i : dmap) {
    ...
    if (rt::thisLocality() == l) {
      ...
      block_contiguous_remote(rma_handle, l, loc_first, loc_last, d_first, op);
    }
    std::advance(loc_first, i.second);
    std::advance(d_first, i.second);
  }

  if (!rma_handle.IsNull()) rt::waitForCompletion(rma_handle);
...
// parallel
template <class ForwardIt1, class ForwardIt2, class UnaryOperation>
ForwardIt2 dpar_kernel(std::true_type, rt::Handle& h, ForwardIt1 first,
                       ForwardIt1 last, ForwardIt2 d_first, UnaryOperation op) {
  using itr_traits1 = distributed_iterator_traits<ForwardIt1>;
  using itr_traits2 = distributed_random_access_iterator_trait<ForwardIt2>;
  auto loc_range = itr_traits1::local_range(first, last);
  auto loc_first = loc_range.begin();
  ...
  for (auto i : dmap) {
    ...
    if (rt::thisLocality() == l) {
      ...
      block_contiguous_remote(h, l, loc_first, loc_last, d_first, op);
    std::advance(loc_first, i.second);
    std::advance(d_first, i.second);
  }
...
template <class ForwardIt1, class ForwardIt2, class UnaryOperation>
ForwardIt2 dpar_kernel(std::false_type, rt::Handle& h, ForwardIt1 first,
                       ForwardIt1 last, ForwardIt2 d_first, UnaryOperation op) {
...
template <class ForwardIt1, class ForwardIt2, class UnaryOperation>
ForwardIt2 dpar_kernel(rt::Handle& h, ForwardIt1 first, ForwardIt1 last,
                       ForwardIt2 d_first, UnaryOperation op) {
  return dpar_kernel(is_block_contiguous<ForwardIt2>::value, h, first, last,
                     d_first, op);
}
...
ForwardIt2 transform(distributed_parallel_tag&& policy, ForwardIt1 first1,
  ...
  auto map_res = distributed_map_init(
      ...
      first1, last1,
      // kernel
      [](rt::Handle& h, ForwardIt1 first1, ForwardIt1 last1, ForwardIt2 d_first,
         UnaryOperation unary_op) {
        return transform_impl::dpar_kernel(h, first1, last1, d_first, unary_op);
      },
      // init value
      d_first,
      // map arguments
...
void generate(distributed_parallel_tag&& policy, ForwardIt first,
  ...
  distributed_map_void(
      ...
      first, last,
      // kernel
      [](rt::Handle& h, ForwardIt first, ForwardIt last, Generator generator) {
        using local_iterator_t = typename itr_traits::local_iterator_type;
...
void replace(distributed_parallel_tag&& policy, ForwardIt first, ForwardIt last,
  ...
  distributed_map_void(
      ...
      first, last,
      // kernel
      [](rt::Handle& h, ForwardIt first, ForwardIt last, const T& old_value,
         const T& new_value) {
        using local_iterator_t = typename itr_traits::local_iterator_type;
...
void replace_if(distributed_parallel_tag&& policy, ForwardIt first,
  ...
  distributed_map_void(
      ...
      first, last,
      // kernel
      [](rt::Handle& h, ForwardIt first, ForwardIt last, UnaryPredicate p,
         const T& new_value) {
        using local_iterator_t = typename itr_traits::local_iterator_type;
